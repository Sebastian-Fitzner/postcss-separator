...
DataSeparator.prototype.postcss = function(opts, css) {
	...
	if (pattern.matchAtRuleType || pattern.matchValue || pattern.matchProp || pattern.matchRule || pattern.matchMedia) {
		...
		if (pattern.matchAtRuleType) {
			css.eachAtRule(function(atRule) {
				if (atRule.name === 'media' && atRule.params.match(pattern.matchAtRuleType)) {
					atRule.removeSelf();
					modCSS.append(atRule);
				} else if (atRule.name.match(pattern.matchAtRuleType)) {
					atRule.removeSelf();
					modCSS.append(atRule);
				}
			});
